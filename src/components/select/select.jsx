...
import StyleSheet from '../../utils/tool/style';
import { Component, PropTypes,View,Transition } from '../../utils';
import { addResizeListener, removeResizeListener } from '../../utils/tool/resizeEvent';
...
class Select extends Component {
  ...
  constructor(props: Object) {
    ...
    this.state = {
      ...
      cachedPlaceHolder: props.placeholder || i18n.t('ko.select.placeholder'),
      currentPlaceholder: props.placeholder || i18n.t('ko.select.placeholder'),
      selectedLabel: '',
      selectedInit: false,
      visible: false,
      selected: undefined,
  ...
  onVisibleChange(visible: boolean) {
    ...
    if (!visible) {
      ...
      if (this.refs.root.querySelector('.ko-input__icon')) {
        const elements = this.refs.root.querySelector('.ko-input__icon');
      ...
      let icon = this.refs.root.querySelector('.ko-input__icon');
  ...
  emptyText(): mixed {
    ...
    if (loading) {
      return i18n.t('ko.select.loading');
    } else {
      if (voidRemoteQuery) {
      ...
      if (filterable && filteredOptionsCount === 0) {
        return i18n.t('ko.select.noMatch');
      }

      if (options.length === 0) {
        return i18n.t('ko.select.noData');
      }
    }
  ...
  render() {
    ...
    return (
      <div ref="root" style={this.style()} className={this.className('ko-select')}>
        {
          multiple && (
            <div ref="tags" className="ko-select__tags" onClick={this.toggleMenu.bind(this)} style={{
            ...
            }}>
              {
                selected.map(el => {
                  return (
                    ...
                    >
                      <span className="ko-select__tags-text">{el.currentLabel()}</span>
                    </Tag>
                  )
                })
              }
        ...
        />
        <Transition name="ko-zoom-in-top" onEnter={this.onEnter.bind(this)} onAfterLeave={this.onAfterLeave.bind(this)}>
          <View show={visible && this.emptyText() !== false}>
            <div ref="popper" className={this.classNames('ko-select-dropdown', {
              ...
              { this.emptyText() && <p className="ko-select-dropdown__empty">{this.emptyText()}</p> }
            </div>
          </View>
        </Transition>
      </div>
